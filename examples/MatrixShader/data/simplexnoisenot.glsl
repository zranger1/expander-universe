...
// original https://www.shadertoy.com/view/wt3GRf
...
void main(void) {
    ...
    vec2 uv = gl_FragCoord.xy / max(resolution.x, resolution.y) * 4.0;
    
    // Input point
    vec3 X = vec3(uv, mod(time, 578.0) * 0.8660254037844386);
    
    // Evaluate noise
    vec4 noiseResult = bccNoisePlaneFirst(X);

    // Time varying pixel col
///    vec3 col = (gl_FragCoord.x > resolution.x / 2.0 ? noiseResult.xyz / 5.5 : noiseResult.www) * 0.5 + 0.5;
